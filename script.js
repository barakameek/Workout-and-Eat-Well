document.addEventListener('DOMContentLoaded', () => {
    ...
    function exportAllData() {
        const allData = {
            workouts: getData(WORKOUT_STORAGE_KEY), routines: getData(ROUTINES_STORAGE_KEY),
            exercises: getData(EXERCISES_STORAGE_KEY), measurements: getData(MEASUREMENTS_STORAGE_KEY),
            nutrition: getData(NUTRITION_STORAGE_KEY), settings: userSettings
        };
    ...
    function importAllDataFile(file) { // Renamed to avoid conflict with HTML element ID
    ...
    function addExerciseToCurrentLogDOM(exercise) {
        const li = document.createElement('li'); li.className = 'active-workout-exercise'; li.dataset.exerciseIndex = activeWorkoutLog.indexOf(exercise); // Use index if object reference changes
        ...
        const exerciseIndex = activeWorkoutLog.indexOf(exercise); // Get current index for listeners
        const setsContainer = li.querySelector('.sets-container');
        setsContainer.addEventListener('input', (e) => { const setRow = e.target.closest('.set-row'); if (!setRow) return; const currentSetIndex = parseInt(setRow.dataset.setIndex); if (e.target.classList.contains('set-weight-input')) activeWorkoutLog[exerciseIndex].loggedSets[currentSetIndex].weight = e.target.value; else if (e.target.classList.contains('set-reps-input')) activeWorkoutLog[exerciseIndex].loggedSets[currentSetIndex].reps = e.target.value; });
        setsContainer.addEventListener('click', (e) => { const logButton = e.target.closest('.log-set-btn'); if(!logButton) return; const setRow = logButton.closest('.set-row'); const currentSetIndex = parseInt(setRow.dataset.setIndex); const currentSet = activeWorkoutLog[exerciseIndex].loggedSets[currentSetIndex]; if (currentSet.weight && currentSet.reps) { currentSet.completed = !currentSet.completed; setRow.classList.toggle('completed', currentSet.completed); logButton.innerHTML = currentSet.completed ? '<i class="fas fa-check-circle"></i> Done!' : '<i class="far fa-circle"></i> Log'; } else if (currentSet.completed) { currentSet.completed = false; setRow.classList.remove('completed'); logButton.innerHTML = '<i class="far fa-circle"></i> Log'; } else { alert("Please enter weight and reps for this set first! 😊"); } });
        li.querySelector('.exercise-session-notes').oninput = (e) => activeWorkoutLog[exerciseIndex].notes = e.target.value;
        li.querySelector('.remove-exercise-from-log-btn').onclick = () => { if(confirm(`Remove ${exercise.name} from this workout? 🤔`)){ activeWorkoutLog.splice(exerciseIndex, 1); renderActiveWorkoutLogDOM(); if (activeWorkoutLog.length === 0 && finishWorkoutBtn) finishWorkoutBtn.style.display = 'none'; }};
    ...
    function findLastPerformance(exerciseName, setNumberTarget) {
        ...
        for (let i = 0; i < allWorkouts.length; i++) { const session = allWorkouts[i]; const exerciseLog = session.exercises.find(ex => ex.name === exerciseName); if (exerciseLog && exerciseLog.log && exerciseLog.log.length >= setNumberTarget) { return exerciseLog.log[setNumberTarget - 1]; }}
        return null;
    }
    ...
    function logRestDayPrompt(dateToLog = null) {
        const restDate = dateToLog || (workoutDateInput ? workoutDateInput.value : null) || new Date().toISOString().split('T')[0];
        ...
        const restDayEntry = { id: generateId(), date: restDate, type: "REST_DAY", notes: notes || "" }; const allEntries = getData(WORKOUT_STORAGE_KEY);
        const existingEntryIndex = allEntries.findIndex(entry => entry.date === restDate);
        if (existingEntryIndex > -1) { if (confirm(`An entry already exists for ${restDate}. Overwrite it with a rest day? 🤔`)) { allEntries.splice(existingEntryIndex, 1, restDayEntry); } else { alert("Rest day not logged."); return; }
        } else { allEntries.push(restDayEntry); }
        allEntries.sort((a, b) => new Date(b.date) - new Date(a.date)); saveData(WORKOUT_STORAGE_KEY, allEntries);
    ...
    function prePopulateExerciseLibrary() { let exercises = getData(EXERCISES_STORAGE_KEY); if (exercises.length > 0 && !localStorage.getItem('forceExerciseRepopulate')) return; const defaultExercises = [ { name: "Barbell Bench Press", primaryMuscle: "Chest", equipment: "Barbell" }, { name: "Standing Overhead Press", primaryMuscle: "Shoulders", equipment: "Barbell" }, { name: "Close-Grip Bench Press", primaryMuscle: "Triceps", equipment: "Barbell" }, { name: "Barbell Front Raise", primaryMuscle: "Shoulders", equipment: "Barbell" }, { name: "Plate Raise", primaryMuscle: "Shoulders", equipment: "Plate" }, { name: "Overhead Triceps Extension (Barbell)", primaryMuscle: "Triceps", equipment: "Barbell" }, { name: "Barbell Back Squat", primaryMuscle: "Quads", equipment: "Barbell" }, { name: "Romanian Deadlift (RDL)", primaryMuscle: "Hamstrings", equipment: "Barbell" }, { name: "Walking Lunges", primaryMuscle: "Quads", equipment: "Bodyweight" }, { name: "Paused Squat", primaryMuscle: "Quads", equipment: "Barbell" }, { name: "Calf Raises (Barbell)", primaryMuscle: "Calves", equipment: "Barbell" }, { name: "Barbell Bent-Over Rows", primaryMuscle: "Back", equipment: "Barbell" }, { name: "Deadlifts", primaryMuscle: "Back", equipment: "Barbell" }, { name: "Barbell Curls", primaryMuscle: "Biceps", equipment: "Barbell" }, { name: "Shrugs (Barbell)", primaryMuscle: "Traps", equipment: "Barbell" }, { name: "Reverse-Grip Rows (Underhand)", primaryMuscle: "Back", equipment: "Barbell" }, { name: "Front Squat", primaryMuscle: "Quads", equipment: "Barbell" }, { name: "Incline Bench Press", primaryMuscle: "Chest", equipment: "Barbell" }, { name: "Power Clean", primaryMuscle: "Full Body", equipment: "Barbell" }, { name: "Push-ups", primaryMuscle: "Chest", equipment: "Bodyweight" }, { name: "Pull-ups", primaryMuscle: "Back", equipment: "Pull-up bar" }, { name: "Dips", primaryMuscle: "Triceps", equipment: "Dip Bars" }, { name: "Plank", primaryMuscle: "Core", equipment: "Bodyweight"}, { name: "Running", primaryMuscle: "Cardio", equipment: "N/A"}, { name: "Cycling", primaryMuscle: "Cardio", equipment: "Bike"} ]; const exercisesWithIds = defaultExercises.map(ex => ({ ...ex, id: generateId(), instructions: `Perform ${ex.name} focusing on good form. ✨`, videoUrl: '', imageUrl: '' })); saveData(EXERCISES_STORAGE_KEY, exercisesWithIds); localStorage.removeItem('forceExerciseRepopulate'); console.log("Default exercises populated."); }
    function prePopulateRoutines() { let routines = getData(ROUTINES_STORAGE_KEY); if (routines.length > 0 && !localStorage.getItem('forceRoutineRepopulate')) return; const defaultRoutines = [ { name: "🌸 Upper Push Day", description: "Chest, Shoulders, Triceps Focus!", exercises: [ { name: "Barbell Bench Press", sets: "4", reps: "6-10", id: generateId() }, { name: "Standing Overhead Press", sets: "4", reps: "6-10", id: generateId() }, { name: "Close-Grip Bench Press", sets: "3", reps: "8-12", id: generateId() } ]}, { name: "🦵 Leg Power Day", description: "Legs & Glutes Blast!", exercises: [ { name: "Barbell Back Squat", sets: "4", reps: "6-10", id: generateId() }, { name: "Romanian Deadlift (RDL)", sets: "3", reps: "8-12", id: generateId() }, { name: "Walking Lunges", sets: "3", reps: "10-15 each", id: generateId() } ]}, { name: "💪 Upper Pull Day", description: "Back, Biceps, Traps Time!", exercises: [ { name: "Barbell Bent-Over Rows", sets: "4", reps: "6-10", id: generateId() }, { name: "Deadlifts", sets: "3", reps: "5-8", id: generateId() }, { name: "Barbell Curls", sets: "3", reps: "10-12", id: generateId() } ]}, { name: "🌟 Full Body Sparkle", description: "Overall Power and Growth!", exercises: [ { name: "Barbell Back Squat", sets: "3", reps: "6-8", id: generateId() }, { name: "Barbell Bench Press", sets: "3", reps: "6-8", id: generateId() }, { name: "Barbell Bent-Over Rows", sets: "3", reps: "6-8", id: generateId() } ]} ]; const routinesWithIds = defaultRoutines.map(r => ({ ...r, id: generateId() })); saveData(ROUTINES_STORAGE_KEY, routinesWithIds); localStorage.removeItem('forceRoutineRepopulate'); console.log("Default routines populated.");}
    ...
    // (This section is now much cleaner as functions are defined above)
    // Theme (already attached)
    // Tabs (already attached)
    // Dashboard (buttons attached in loadDashboardData)
    // Log Workout
    if(exerciseForm) exerciseForm.addEventListener('submit', function(event) { /* ... as defined before ... */});
    if(finishWorkoutBtn) finishWorkoutBtn.addEventListener('click', function() { /* ... as defined before ... */ });
    ...
    function initializeApp() {
        console.log("🚀 Initializing Cute Workout Buddy! 🌸 v2.2");
        loadUserSettings();
